...
def create_new_table(experiment_num, results):
    """Create new table with first experiment."""
    # Single-line header with Exp#.Method# format
    header = f"| Sample | {experiment_num}.1 | {experiment_num}.2 | {experiment_num}.3 | {experiment_num}.4 |"
    separator = "|--------|---------|---------|---------|---------|"
...
def append_to_table(existing_table, experiment_num, results):
    """Append experiment columns to existing table."""
    header, separator, rows = existing_table

    # Append new experiment columns with Exp#.Method# format
    new_header = header.rstrip('|').rstrip() + f" | {experiment_num}.1 | {experiment_num}.2 | {experiment_num}.3 | {experiment_num}.4 |"
...
def main():
    if len(sys.argv) < 4:
        print("Usage: append_experiment.py <results_md> <output_file> <experiment_num>")
        print("Example: append_experiment.py tmp/exp1.md results.md Exp0")
        sys.exit(1)

    results_file = sys.argv[1]
    output_file = sys.argv[2]
    experiment_num = sys.argv[3]  # e.g., "Exp0", "Exp1", etc.
    ...
    if os.path.exists(output_file):
        ...
        if table_data:
            ...
            new_table = append_to_table(table_data, experiment_num, results)
            ...
            new_table = create_new_table(experiment_num, results)
    ...
    print(f"Updated {output_file} with {experiment_num}")
